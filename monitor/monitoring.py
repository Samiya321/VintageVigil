    ...
    with logger.contextualize(
        ...
        try:
            ...
            for item in database.upsert_products(
                    products_to_process, search_query.keyword, website_config.website_name
            ):
...
async def monitor_site(
        site_config, database, notification_clients, user_dir, is_running
):
    """
    ...
    iteration_count = 0
    while is_running:
        scraper = fetch_scraper(site_config.common.website_name)
        ...
        await asyncio.sleep(site_config.common.delay)
        iteration_count += 1
